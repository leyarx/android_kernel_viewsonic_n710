/*
 * arch/arm/mach-tegra/board-n710-misc.c
 *
...
static unsigned int n710_pcbid;

unsigned int grouper_query_pcba_revision(void)
{
	return n710_pcbid & 0x7;
}
...
unsigned int grouper_query_pmic_id(void)
{
	...
	value = (n710_pcbid & 0x8) >> 3;

	return value;
}
EXPORT_SYMBOL(grouper_query_pmic_id);

static ssize_t n710_chipid_show(struct kobject *kobj,
...
static ssize_t n710_pcbid_show(struct kobject *kobj,
	struct kobj_attribute *attr, char *buf)
{
	char *s = buf;
	int i;
	...
	for (i = 4; i > 0; i--)
		s += sprintf(s, "%c", n710_pcbid & (1 << (i - 1)) ? '1' : '0');
	s += sprintf(s, "b\n");
	return (s - buf);
}

N710_MISC_ATTR(n710_chipid);
N710_MISC_ATTR(n710_pcbid);

static struct attribute *attr_list[] = {
	&n710_chipid_attr.attr,
	&n710_pcbid_attr.attr,
	NULL,
};
...
static struct platform_device *n710_misc_device;
...
static int pcbid_init(void)
{
	...
	n710_pcbid = ((gpio_get_value(TEGRA_GPIO_PK3) << 3 ) |
...
int __init n710_misc_init(void)
{
	int ret = 0;
	...
	n710_misc_device = platform_device_alloc("n710_misc", -1);
	...
	ret = platform_device_add(n710_misc_device);
	if (ret) {
		pr_err("[MISC]: cannot add device to platform.\n");
		goto fail_platform_add_device;
	}

	ret = sysfs_create_group(&n710_misc_device->dev.kobj, &attr_group);
	if (ret) {
...
fail_sysfs:
	platform_device_del(n710_misc_device);

fail_platform_add_device:
	platform_device_put(n710_misc_device);
