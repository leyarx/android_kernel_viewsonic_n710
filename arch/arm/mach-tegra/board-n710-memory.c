...
int n710_emc_init(void)
{
	...
	if (ret < 0) {
		printk("n710_emc_init: request MEMORY_BOOSTRAP_PIN_AD4 failed\n");
		WARN_ON(1);
		goto err_handle;
	}
	...
	if (ret < 0) {
		printk("n710_emc_init: failed to configure MEMORY_BOOSTRAP_PIN_AD4\n");
		WARN_ON(1);
		goto err_handle;
	}
	...
	if (ret < 0) {
		printk("n710_emc_init: request MEMORY_BOOSTRAP_PIN_AD5 failed\n");
		WARN_ON(1);
		goto err_handle;
	}
	...
	printk("n710_emc_init:mem_bootstrap_ad4=%u mem_bootstrap_ad5=%u \n",mem_bootstrap_ad4,mem_bootstrap_ad5);
	...
	if(!mem_bootstrap_ad4 && !mem_bootstrap_ad5){
		...
		printk("n710_emc_init:Nakasi_dvfs_Elpida_table_0430\n");
	}else{
		tegra_init_emc(Nakasi_dvfs_Hynix_table_0430,
			ARRAY_SIZE(Nakasi_dvfs_Hynix_table_0430));
		printk("n710_emc_init:Nakasi_dvfs_Hynix_table_0430\n");
	}
...
err_handle:
	if(tegra_init_emc(Nakasi_dvfs_Elpida_table_0430,
		...
		printk("n710_emc_init:no validate EMC tabe, disable EMC DVFS\n");
	return 0;
}
