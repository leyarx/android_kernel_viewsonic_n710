/*
 * arch/arm/mach-tegra/board-n710-power.c
 *
...
static int __init n710_max77663_regulator_init(void)
{
...
static int __init n710_fixed_regulator_init(void)
{
	int i;
...
subsys_initcall_sync(n710_fixed_regulator_init);

int __init n710_regulator_init(void)
{
	void __iomem *pmc = IO_ADDRESS(TEGRA_PMC_BASE);
	u32 pmc_ctrl;
	int ret;
	...
	ret = n710_max77663_regulator_init();
	if (ret < 0)
		return ret;
...
static void n710_board_suspend(int lp_state, enum suspend_stage stg)
{
	if ((lp_state == TEGRA_SUSPEND_LP1) && (stg == TEGRA_SUSPEND_BEFORE_CPU))
		tegra_console_uart_suspend();
}

static void n710_board_resume(int lp_state, enum resume_stage stg)
{
	if ((lp_state == TEGRA_SUSPEND_LP1) && (stg == TEGRA_RESUME_AFTER_CPU))
		tegra_console_uart_resume();
}

static struct tegra_suspend_platform_data n710_suspend_data = {
	.cpu_timer	= 2000,
	.cpu_off_timer	= 200,
	...
	.board_suspend = n710_board_suspend,
	.board_resume = n710_board_resume,
};

int __init n710_suspend_init(void)
{
	tegra_init_suspend(&n710_suspend_data);
	return 0;
}
